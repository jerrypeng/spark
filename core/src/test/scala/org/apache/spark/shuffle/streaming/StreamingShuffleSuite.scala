...
class StreamingShuffleSuite
  ...
  /**
   * Mock reader subclass kept for parity with the runtime test suite. Currently a thin wrapper
   * over StreamingShuffleReader.
   */
  ...
  test("SC-165888: Fix race condition between client" +
    " creation and reader termination in shuffle reader") {
    withSpark(new SparkContext("local", "StreamingShuffleSuite", sparkConf)) { sc =>
      val g = new ShuffleGroup[Int](sc, 2, 1)
  ...
  test("RemoveShuffle unregisters the shuffle from the streaming shuffle output tracker") {
    withSpark(new SparkContext("local", "StreamingShuffleSuite", sparkConf)) { sc =>
      val tracker = SparkEnv.get.streamingShuffleOutputTracker.get
        .asInstanceOf[StreamingShuffleOutputTrackerMaster]
      tracker.registerShuffle(shuffleId, numMaps = 1, numReduces = 1, jobId = 0)
      assert(tracker.getShuffleInfo(shuffleId).isDefined)

      // Removing the shuffle must free the tracker's per-shuffle state via
      // BlockManagerStorageEndpoint's RemoveShuffle handler.
      sc.env.blockManager.master.removeShuffle(shuffleId, blocking = true)
